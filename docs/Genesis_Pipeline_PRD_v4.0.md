...
- Native mobile app — PWA delivered in Phase 10 (`/m/` routes, 5-tab bottom nav, web push) + Phase 10.1 (Conversations/Inbox tab with SMS threads). PWA navigation performance optimized — loading states + cached auth eliminate ~3s tab-switch delays. Native App Store distribution (Capacitor wrapper) planned as Phase 11 when user base grows beyond internal team.
...
| **SHOULD** | HCP real-time webhooks | Webhook receiver at `/api/webhooks/hcp` handling 10 HCP events: customer sync (updated/deleted), estimate lifecycle (created→log, completed→import, sent, updated), option events (approval_status_changed, created), job linking (copy_to_job), real-time commission confirmation (job.paid). Supplements polling. v0.2 — built and deployed. **Status:** Verified working with live events. Handler uses Next.js `after()` for background execution, guaranteeing sub-second response times. |
...
| Twilio A2P 10DLC registration | Required for business SMS compliance. Three campaign submissions rejected (attempts 1–3, Feb 2025–Feb 2026) — failures due to vague MESSAGE_FLOW description, missing CTIA-compliant disclosures, and privacy/terms page content gaps. Attempt 4 pending with new `/sms-consent` page providing verbal opt-in CTA and full CTIA disclosures (message frequency, "Msg & data rates may apply", STOP/HELP instructions). Without approval, US carriers silently drop outbound SMS (error 30034). |
...
| Campaigns | Marketing Campaigns — Email & SMS (C1-C8) | **In progress** — C1-C7 complete (schema, unsubscribe, email builder, audience/wizard, execution engine, dashboard/analytics, SMS campaigns), next: C8 polish + settings |
