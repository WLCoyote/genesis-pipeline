...
| **SHOULD** | HCP real-time webhooks | Webhook receiver at `/api/webhooks/hcp` handling 10 HCP events: customer sync (updated/deleted), estimate lifecycle (created→log, completed→import, sent, updated), option events (approval_status_changed, created), job linking (copy_to_job), real-time commission confirmation (job.paid). Supplements polling. v0.2 — built and deployed. **Status:** The webhook handler now uses Next.js `after()` for background execution, guaranteeing sub-second response times and resolving HCP's auto-disable behavior caused by slow responses. Previously, HCP auto-disabled the webhook subscription due to a failed delivery backlog (events queued before the endpoint was live); re-enable in HCP My Apps → Webhooks if still disabled. |
