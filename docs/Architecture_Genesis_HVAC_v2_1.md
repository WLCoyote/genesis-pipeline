...
**v3.0 Changes:** Full sequence timeline rewrite — shows all steps from assigned sequence with merged event history. Added Skip Step API (POST /api/estimates/[id]/skip-step) and button. Sequence pause/resume via new `is_active` column on `follow_up_sequences` (SQL 011) with admin toggle in sequence editor. Cron Phase 2 now validates step exists in current sequence and sequence is active before sending pending events — prevents stale sends after edits. Timeline preserves orphaned historical events and uses event's actual channel for executed steps. Previous: v2.9 fixed HCP polling accuracy. v2.8 rewrote HCP polling. v2.7 added Send Now. v2.6 added Update Estimates, admin delete, lead archiving. v2.5 corrected estimate pipeline entry flow. v2.4 added SMS Inbox. v2.3 added team management. v2.2 added Flow 2, leads, estimate links, dark mode. v2.1 added Twilio Hosted SMS.
...
**New estimate detection:** If an estimate exists in HCP but not locally, and any option has `status = "submitted for signoff"` (sent to customer) or `approval_status = "approved"/"declined"` (already resolved), the cron creates the local customer and estimate. Customer name priority: `customer.company` > `first_name + last_name` > "Unknown". HCP amounts are in cents — divided by 100. Sent date is derived from `option.updated_at` for submitted estimates, or `estimate.schedule.scheduled_start` for resolved ones. Local estimate status maps from HCP: submitted → "active" (enrolled in sequence), approved → "won", all declined → "lost" (no sequence). This is how estimates from both Flow 1 and Flow 2 enter the pipeline.

**Existing estimate updates:** Full refresh on every poll. Customer name, email, phone are updated from HCP. Estimate total\_amount (cents/100), sent\_date, and URL are refreshed. All option amounts, descriptions, and statuses are synced. New options added in HCP are created locally. If an option's `approval_status` changes to approved/declined, the estimate status updates to won/lost, the follow-up sequence stops, and the assigned comfort pro is notified.
