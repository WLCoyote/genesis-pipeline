...
**v2.9 Changes:** Fixed HCP polling accuracy: sent detection uses `option.status = "submitted for signoff"` (not `approval_status`). Also imports already-resolved estimates (approved/declined). Customer name priority: `customer.company` > first+last > Unknown. HCP amounts in cents — divided by 100. Sent date from `option.updated_at` (submitted) or `estimate.schedule.scheduled_start` (resolved). Existing estimates get full refresh (customer, amounts, options, sent_date). Estimates sorted newest-first. Removed `scheduled_start_min/max` API filters (filter by appointment date, not creation); now fetches newest-first and filters by `created_at` in code. Previous: v2.8 rewrote HCP polling with pre-fetched index, page caps, correct API params. v2.7 added Send Now, moved-to-HCP archived, total amount fix. v2.6 added Update Estimates, admin delete, lead archiving. v2.5 corrected estimate pipeline entry flow. v2.4 added SMS Inbox. v2.3 added team management. v2.2 added Flow 2, leads, estimate links, dark mode. v2.1 added Twilio Hosted SMS.
