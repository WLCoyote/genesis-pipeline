...
| **follow\_up\_sequences** | id, name, is\_default (boolean) is\_active (boolean, default true) steps (JSONB array of objects):   { day\_offset, channel, template,     is\_call\_task } created\_by, created\_at | Company-level sequence templates. Admin-managed. `is_active` controls whether follow-ups execute — admin can pause/resume without losing step configuration. Steps define the multi-channel follow-up cadence. Templates support placeholders: {{customer\_name}}, {{customer\_email}}, {{comfort\_pro\_name}}, {{estimate\_link}}. Email templates use HTML hyperlinks for estimate links; SMS templates show URL on its own line. |
...
* Follow-up sequence templates use {{estimate\_link}} which resolves to the estimate's online\_estimate\_url if set. Note: HCP API does not expose the customer-facing URL (opaque hash), so this field is not auto-populated by polling and must be set manually if needed.
...
Flow 2 (Built): External leads arrive via /api/leads/inbound webhook. CSRs qualify leads and "Move to HCP" creates the customer and estimate in Housecall Pro via POST /customers and POST /estimates API calls. Move to HCP does NOT create a local estimate — it only updates the lead status to "converted" and stores the hcp\_customer\_id. The estimate enters the pipeline later when the HCP polling cron detects it has been sent to the customer (option status = "submitted for signoff").

## **5.2 Follow-Up Sequence Execution**

A Vercel cron job runs multiple times daily (e.g., every 2 hours during business hours). For each active estimate, it checks: is the sequence active (`is_active = true`)? Is a sequence step due (based on sent\_date \+ step day\_offset)? Is the estimate snoozed (skip if snooze\_until is in the future)? Has the estimate been won/lost/dormant (stop if so)?

For auto-send steps (email/SMS): the system creates a follow\_up\_event with status "pending\_review" and a scheduled\_at 30 minutes in the future. Before sending, the cron validates the step still exists in the current sequence and the sequence is still active — if the sequence was edited or paused between scheduling and sending, the event is marked "skipped" instead. If valid, the cron sends via Resend (email) or Twilio (SMS), updating the status to "sent." Outbound SMS messages are also logged to the messages table for conversation tracking.

Alternatively, the estimate detail page shows a "Send Now" button (POST /api/estimates/[id]/send-next) when a sequence step is due but hasn't been scheduled yet. This sends the step immediately — bypassing the 30-minute edit window — and is designed for the workflow where you send an estimate in HCP, click "Update Estimates" in Genesis to pull it in, then immediately send the Day 0 SMS. A "Skip Step" button (POST /api/estimates/[id]/skip-step) allows manually advancing past the current step without sending it. Skipped steps in the timeline show an "Execute" button (POST /api/estimates/[id]/execute-step) that sends the step after the fact — useful when a step was skipped due to late import or manual skip but should still be delivered.

When the sequence is paused (`is_active = false`), the estimate detail page shows a yellow "follow-ups on hold" banner in the actions panel. Send Now, Skip Step, and pending event editing are hidden. Timeline shows "Paused" badges on all incomplete steps.
...
The estimate detail page shows a full sequence timeline — all steps from the assigned sequence with their status. Steps with events show the actual event status (Sent, Opened, Skipped, etc.). Steps before the current index without events show as "Skipped" (estimate imported late). The current step is highlighted. Future steps show as "Upcoming" with projected due dates. Won/lost/dormant estimates show remaining steps as "Not Reached". When the sequence is edited, the timeline immediately reflects changes for future steps while preserving historical event data.

## **5.3 HCP Status Sync**

A Vercel cron job runs 3 times daily, and admin/CSR users can trigger polling on demand via the "Update Estimates" button (POST /api/admin/update-estimates). Both use the same shared polling logic in `lib/hcp-polling.ts`. The system calls GET /estimates on the HCP API with `page_size=200` and `sort_direction=desc` (newest first). No API date filters are used — HCP's `scheduled_start_min/max` filters by appointment date, not creation date. Instead, estimates are filtered in code by comparing `created_at` against the auto\_decline\_days cutoff. Pagination stops early when an entire page is older than the cutoff. Max 5 pages. Before fetching from HCP, all local estimate IDs (`hcp_estimate_id` and `estimate_number`) are pre-loaded into memory Sets for O(1) matching. Estimates are processed page-by-page. For each returned estimate, it performs two functions:

**New estimate detection:** If an estimate exists in HCP but not locally, and any option has `status = "submitted for signoff"` (sent to customer) or `approval_status = "approved"/"declined"` (already resolved), the cron creates the local customer and estimate. Customer name priority: `customer.company` > `first_name + last_name` > "Unknown". HCP amounts are in cents — divided by 100. Sent date is derived from `estimate.schedule.scheduled_start` if available, else `estimate.created_at` (`option.updated_at` was removed — it reflects last modification, not send date). Local estimate status maps from HCP: submitted → "active" (enrolled in sequence), approved → "won", all declined → "lost" (no sequence). This is how estimates from both Flow 1 and Flow 2 enter the pipeline.

**Existing estimate updates:** Full refresh on every poll. Customer name, email, phone are updated from HCP. Estimate total\_amount (cents/100) and sent\_date are refreshed. All option amounts, descriptions, and statuses are synced. New options added in HCP are created locally. If an option's `approval_status` changes to approved/declined, the estimate status updates to won/lost, the follow-up sequence stops, and the assigned comfort pro is notified.

**Mark Won/Lost with option selection:** The estimate detail page shows Mark Won and Mark Lost buttons (secondary/outlined style — manual overrides, since customers approve by signing in HCP). Clicking either opens an inline option selection modal with checkboxes for each pending option. Won: selected options marked approved locally (no HCP API call — customer signs in HCP), unselected pending options declined in HCP via POST /estimates/options/decline. Lost: selected options declined in HCP, if all options are now declined the estimate moves to lost. The API route POST /api/estimates/[id]/status accepts `{ action: "won"|"lost", selected_option_ids: [...] }`. When an estimate moves to won/lost, all remaining follow-up events (scheduled, pending_review, snoozed) are marked skipped.

Auto-decline processing is handled separately by the /api/cron/auto-decline job.

## **5.4 Inbound SMS (Twilio Webhook)**

When a customer replies to an SMS, Twilio sends a webhook POST to /api/webhooks/twilio. The handler: 1) Validates the Twilio request signature. 2) Matches the incoming phone number to a customer record. 3) Creates a row in the messages table with direction "inbound." 4) If the customer has an active estimate, links the message to that estimate via estimate_id. 5) Creates a notification (type: sms_received) for the comfort pro assigned to the active estimate. 6) Always notifies all active admins and CSRs (type: sms_received for matched messages, unmatched_sms for unknown numbers), with deduplication so an assigned user who is also admin only gets one notification. 7) Supabase Realtime pushes the new message to the frontend, updating the conversation thread live.
...
The HCP API does not expose the customer-facing estimate URL (it uses an opaque hash like `https://client.housecallpro.com/estimates/{hash}` that is not derivable from API data). The `online_estimate_url` field on the estimates table exists for manual entry but is not auto-populated by HCP polling. The {{estimate\_link}} placeholder in follow-up templates resolves to this field if set — in email templates as a styled "View Your Estimate" button, in SMS templates as a URL on its own line. The estimate number in the detail header is clickable and links to the HCP pro view at `https://pro.housecallpro.com/app/estimates/{option_id}` (using the first option's hcp\_option\_id).
