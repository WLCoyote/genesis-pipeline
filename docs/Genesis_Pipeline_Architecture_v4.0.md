...
| `/api/webhooks/hcp` | POST | HMAC-SHA256 via `HCP_WEBHOOK_SECRET` (optional — skipped if env var not set). Returns 200 immediately; all event processing runs in Next.js `after()` for background execution to satisfy HCP's 5-second timeout (prevents HCP from auto-disabling the webhook due to slow responses). Returns 200 for HCP test ping (`{"foo":"bar"}`) before signature check. 10 HCP events: customer.updated/deleted, estimate.created (log only)/completed (primary import)/sent/updated, estimate.option.approval_status_changed/created, estimate.copy_to_job, job.paid (commission confirm). Payload structure: entity IDs at `event.estimate.id`, `event.customer.id`, `event.job.id` (not `event.data.*`). Fetches full entity from HCP API, delegates to handler functions. Verified working with live HCP events. (v0.2) |
...
| `/app/components/ui/` | Design system component library (Phase 8.3). 7 reusable primitives: Button (7 variants × 4 sizes), PageTopbar, Card, SectionHeader, StatCard, Modal, FormField (+ inputCls/selectCls/textareaCls constants). Used across 45+ files. |
