...
| v0.2 | HCP Real-Time Webhooks — `/api/webhooks/hcp` receiver for 10 events (customer.updated/deleted, estimate.created/completed/sent/updated, option.approval_status_changed/created, estimate.copy_to_job, job.paid). sql/034 adds hcp_job_id + job payment fields. job.paid confirms commission in real-time. | **Complete** — sql/034 run in Supabase. Code deployed. Event processing uses `after()` for instant 200 response (prevents HCP 5-second timeout auto-disable). Old retry backlog may still need HCP support to clear. |
